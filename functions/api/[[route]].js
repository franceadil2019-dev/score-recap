export async function onRequest(context) {
  ...
  // استخراج الإجراء من الهيدر أو من نهاية المسار (Path)
  ...
  async function getFromApiSports(endpoint, kvKey, ttlSeconds) {
    // 1. قراءة البيانات من الكاش إن وجدت
    if (env.SPORTS_KV) {
      try {
    ...
    // 2. الاتصال بـ API-Sports في حال عدم وجود كاش
    const res = await fetch(`https://v3.football.api-sports.io/${endpoint}`, {
      headers: { "x-apisports-key": env.API_SPORTS_KEY }
    });
    const dataText = await res.text();

    // 3. تخزين الاستجابة في KV إذا كانت ناجحة
    try {
      const dataObj = JSON.parse(dataText);
      if (env.SPORTS_KV && res.ok && (!dataObj.errors || Object.keys(dataObj.errors).length === 0)) {
        const safeTtl = Math.max(ttlSeconds, 60); // الحد الأدنى للـ TTL هو 60 ثانية
  ...
  try {
    // 1. جلب قائمة المباريات حسب التاريخ
    if (action.includes("fixtures") || action.includes("مباريات")) {
      const date = url.searchParams.get("date") || new Date().toISOString().split('T')[0];
      const today = new Date().toISOString().split('T')[0];
      const ttl = (date === today) ? 60 : 86400; // 60 ثانية لليوم، 24 ساعة للأيام الأخرى
      return await getFromApiSports(`fixtures?date=${date}`, `api_fixtures_${date}`, ttl);
    }

    // 2. جلب الأحداث (Events)
    if (action.includes("events") || action.includes("أحداث")) {
    ...
    if (action.includes("statistics") || action.includes("إحصائيات")) {
      const fixtureId = url.searchParams.get("fixture") || url.searchParams.get("fixtureId");
      if (!fixtureId) return new Response(JSON.stringify({ error: "Missing fixture ID" }), { status: 400, headers: corsHeaders });
      return await getFromApiSports(`fixtures/statistics?fixture=${fixtureId}`, `api_stats_${fixtureId}`, 60);
    }

    // 4. التوقعات الذكية (Pre-Match AI Prediction)
    if (action.includes("predict") || action.includes("توقع")) {
      ...
      // 🚨 تم التعديل إلى النسخة المتوافقة مع حسابك gemini-3.1-pro-preview
    ...
    // 5. تقرير المباراة (Post-Match AI Recap)
    if (action.includes("generate-article") || action.includes("تقرير")) {
      const leagueId = url.searchParams.get("leagueId");
      if (leagueId && leagueId !== "39") {
      ...
      if (env.SPORTS_KV) {
        waitUntil(env.SPORTS_KV.put(kvKey, articleText, { expirationTtl: 604800 })); // حفظ التقرير لمدة أسبوع
      }
